...
Page({
  ...
  checkSingle: function(event){
    ...
    for (var i = 0; i < cart_list.length; i++) {
      if (i == index) {
        ...
        var cartId = cart_list[i].id;
        wx.request({
          url: basePath + 'setItemSelected.action?cartId=' + cartId, //仅为示例，并非真实的接口地址
          header: {
            'content-type': 'application/json', // 默认值
            "sessionId": getApp().getSessionId()
          },
  ...
  checkAll: function (event) {
    ...
    var basePath = getApp().globalData.basePath;
    wx.request({
      url: basePath + 'checkAllCartItem.action?checkAll=' + checked_all, //仅为示例，并非真实的接口地址
      header: {
        'content-type': 'application/json', // 默认值
        "sessionId": getApp().getSessionId()
      },
  ...
  addCount: function(event){
    ...
    for (var i = 0; i < cart_list.length; i++) {
      if (i == index) {
        wx.request({
          url: basePath + 'addCartItemCount.action?cartId=' + cart_list[i].id, //仅为示例，并非真实的接口地址
          header: {
            'content-type': 'application/json', // 默认值
            "sessionId": getApp().getSessionId()
          },
  ...
  inputCount: function(event){
    ...
    for (var i = 0; i < cart_list.length; i++) {
      if (i == index) {
        ...
        wx.request({
          url: basePath + 'setCartItemCount.action?cartId=' + cart_list[i].id + "&count=" + count, //仅为示例，并非真实的接口地址
          header: {
            'content-type': 'application/json', // 默认值
            "sessionId": getApp().getSessionId()
          },
  ...
  desCount: function(event){
    ...
    for (var i = 0; i < cart_list.length; i++) {
      if (i == index) {
        ...
        wx.request({
          url: basePath + 'descCartItemCount.action?cartId=' + cart_list[i].id, //仅为示例，并非真实的接口地址
          header: {
            'content-type': 'application/json', // 默认值
            "sessionId": getApp().getSessionId()
          },
  ...
  submitCart: function(){
    ...
    wx.navigateTo({
      url: '/pages/order/submitOrder?orig=fromCart',
    })
    /*
    //创建订单
